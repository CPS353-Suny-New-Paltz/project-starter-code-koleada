...
public class Checkpoint2TestSuite {
  ...
  public void checkPrototypesExist(Class<? extends Annotation> apiAnnotation,
        ...
        for (Method m : clazz.getDeclaredMethods()) {
          if (m.isAnnotationPresent(prototypeAnnotation)) {
            ...
            if (parameters.length != 1) {
              errors.add(
                  "Prototype method should have a single parameter, the API being prototyped. "
    ...
    if (numPrototypesFound != 1) {
      errors.add("No (or more than one) class method with the "
          + prototypeAnnotation.getSimpleName()
  ...
  @ParameterizedTest(name = ParameterizedTest.ARGUMENTS_PLACEHOLDER)
  @ValueSource(classes = {NetworkAPI.class, ProcessAPI.class,
      ConceptualAPI.class})
  public void checkAnnotationsExist(Class<? extends Annotation> apiAnnotation)
    ...
    if (numApisFound != 1) {
      errors.add("No (or more than one) interface found with annotation "
          + apiAnnotation.getSimpleName() + " in 'src'");
      errors.add(
          "Keep in mind that all APIs must be interfaces (not classes), and that each API annotation"
