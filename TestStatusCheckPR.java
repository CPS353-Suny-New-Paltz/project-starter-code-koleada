...
public class TestStatusCheckPR {
  ...
  public void testPullRequest() throws Exception {
    ...
    for (JsonElement pr : JsonParser.parseString(pullRequests).getAsJsonArray()
        .asList()) {
  ...
  private String getBaseApiPath() throws Exception {
    Process getRemote = new ProcessBuilder("git", "remote", "get-url", "origin",
        "--push").start();
    getRemote.waitFor();
  ...
  private boolean hasReviewerApproval(String baseApiPath, String prNumber)
      throws Exception {
    ...
    for (JsonElement review : JsonParser.parseString(reviewResult)
      ...
      if (review.getAsJsonObject().get("state").getAsString()
          .equals(APPROVED)) {
        return true;
      }
    }
  ...
  private boolean hasStatusChecks(String baseApiPath, String prNumber)
      throws Exception {
    ...
    Map<String, String> firstCommitStatus = getStatusCheckResult(baseApiPath,
        firstCommit);
    ...
    for (JsonElement commit : commits) {
      Map<String, String> statusCheckResult = getStatusCheckResult(baseApiPath,
          commit);
  ...
  private Map<String, String> getStatusCheckResult(String baseApiPath,
      JsonElement commit) throws Exception {
