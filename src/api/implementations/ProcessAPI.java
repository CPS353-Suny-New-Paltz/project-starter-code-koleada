...
public class ProcessAPI implements ProcessApi {
	...
	public LoadResponse load(LoadRequest request) {
		...
		return new LoadResponse(ApiStatus.ERROR, new ArrayList<>(), Delimiter.defaultDelimiter(),
	...
	public StoreResponse store(StoreRequest request) {
		...
		return new StoreResponse(ApiStatus.ERROR, dest, "Unsupported resource type or missing URI");
	}

	/**
	 * Helper method to load data from a file
	 * 
	 * @param src,      Resource to read from
	 * @param delimiter used to split data and add it to a List
	 * @return
	 */
	private LoadResponse loadFromFile(Resource src, Delimiter delimiter) {
		try {
			...
			List<Integer> data = Arrays.stream(tokens).map(String::trim).filter(s -> !s.isEmpty())
					.map(Integer::parseInt).collect(Collectors.toList());

			return new LoadResponse(ApiStatus.SUCCESS, new ArrayList<>(data), Delimiter.defaultDelimiter(),
					"Loaded successfully");
	 ...
	 * 
	 * @param dest      the Resource which describes the file we are to write to
	 * @param batch     DataBatch<List> the data we write to the file
	 * @param delimiter added in between each data element
	 * @return
	 */
	private StoreResponse storeToFile(Resource dest, List batch, Delimiter delimiter) {
		try {
			List<?> payload = batch;
			String joined = payload.stream().map(Object::toString).collect(Collectors.joining(delimiter.getValue()));

			Files.writeString(Paths.get(dest.getUri()), joined);

			return new StoreResponse(ApiStatus.SUCCESS, dest, "Stored successfully");
		} catch (IOException e) {
			return new StoreResponse(ApiStatus.ERROR, dest, "Failed to store: " + e.getMessage());
		}
